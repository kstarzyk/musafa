...
module.exports = config => {
  ...
  .catch(err => {
    if (err.code === '42P01')
    {
      out(config.migrations_table_name + ' table doesn\'t exist')
      out(config.migrations_table_name + ' will be created after first migration')
      const m_db = []
      return {m_local,m_db,m_pending:m_local}
    }
